<template>
  <v-row justify="center">
    <v-dialog v-model="localadd" scrollable max-width="536px">
      <v-card
        min-height="200px"
        style="border-radius: 10px; box-shadow: 4px 4px 4px 4px rgba(0, 0, 0, 0.5);;"
      >
        <v-card-title justify-center style="font-size:28px;">
          Polls Report
          <v-spacer></v-spacer>
        </v-card-title>
        <v-divider></v-divider>
        <v-card-text>
          ...
          <v-card outlined v-for="(poll, i) in pollist" :key="i">
            <!-- <v-row class="mt-n6">
              <v-col cols="12" md="12">
                <v-radio-group v-model="poll.isSingle"  class="float-right">
              ...
              <v-col cols="12" class="pl-6 pr-6">
                <v-row no-gutters>
                  ...
                  <v-col cols="12" md="11" class="ml-n5">
                    <v-text-field dense outlined placeholder="Question" v-model="poll.content" style="font-size: 12px;"></v-text-field>
                  </v-col>
                </v-row>
                <v-row no-gutters>
          ...
          <v-row>
            <v-col cols="12" md="12">
              <div class="float-right">
                <v-btn
                  style="background-color:#ffffff; border:solid 1px #000000; border-radius:10px;"
                  @click="closedialog"
                >Close</v-btn>&nbsp;
                <v-btn
                  style="background-color:#086AAB; border-radius:10px; color:white"
                  @click="inserpoll"
                >Save</v-btn>
              </div>
            </v-col>
...
export default {
  ...
  computed: {
    ...mapState('dialog', ['addPolls']),
    localadd: {
      get () {
        return this.addPolls
      },
      set (value) {
        this.setDialogAddPolls(value)
      }
    }
  },
  methods: {
    ...mapActions('dialog', ['setDialogAddPolls']),
    goPage (link) {
    ...
    async inserpoll () {
      ...
      this.axios
        .post(process.env.VUE_APP_API + '/api/votes_manage/insertpoll', {
          topic,
          pollist
        })
        .then((res) => {
    ...
    async startroom () {
      ...
      var getToken = this.$cookies.get('user-token')
      var token = atob(getToken)
